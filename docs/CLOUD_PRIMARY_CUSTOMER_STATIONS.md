...
The one-time pairing code generated by an authenticated business account is the enrollment proof. The Customer app never receives the owner's password or Auth session, and it does not require the owner email to be retyped.
...
6. Enter the one-time pairing code and connect the PC. The code already identifies the business, branch, and logical PC.
