...
async function downloadReport(reportId){
  ...
  const cards=[['Gross income',money(report.summary.gross)],['Refunds',money(report.summary.refunds)],['Expenses',money(report.summary.expenses)],['Net income',money(report.summary.net)]];cards.forEach((item,index)=>{const x=14+index*45;doc.setFontSize(7);doc.setTextColor(110);doc.text(item[0].toUpperCase(),x,57);doc.setFontSize(11);doc.setFont('helvetica','bold');doc.setTextColor(20);doc.text(item[1],x,65);doc.setFont('helvetica','normal')})
  ...
  const earningsRail = <>
    <AdminRailCard title="Income snapshot" subtitle="Important financial totals for the selected period.">
      <div className="space-y-2">
        ...
        <div className="admin-rail-stat"><span className="text-slate-soft">Refunds</span><b className="stat-figure text-ember-dim">{money(data?.summary?.refunds)}</b></div>
    ...
    <AdminRailCard title="Period context" subtitle="Current reporting window and accounting note.">
      ...
      <p className="mt-3 text-[9px] leading-4 text-slate-soft">Gross income follows services/products actually earned. Wallet top-ups remain stored customer credit until that value is spent.</p>
    </AdminRailCard>
  </>
