...
import { CalendarDays, CircleDollarSign, Download, Plus, ReceiptText, RefreshCw, TriangleAlert, TrendingUp, WalletCards } from 'lucide-react'
...
async function downloadReport(reportId){
  ...
  const cards=[['Gross',money(report.summary.gross)],['Expenses',money(report.summary.expenses)],['Net income',money(report.summary.net)]];cards.forEach((item,index)=>{const x=14+index*60;doc.setFontSize(7);doc.setTextColor(110);doc.text(item[0].toUpperCase(),x,57);doc.setFontSize(11);doc.setFont('helvetica','bold');doc.setTextColor(20);doc.text(item[1],x,65);doc.setFont('helvetica','normal')})
...
export default function EarningsPage(){
  ...
  useEffect(()=>{void load();return()=>{loadSequenceRef.current += 1}},[load])
  useEffect(()=>{
  ...
  const earningsRail = <>
    <AdminRailCard title="Income snapshot" subtitle="Important financial totals for the selected period.">
      <div className="space-y-2">
        <div className="admin-rail-stat"><span className="text-slate-soft">Gross cash</span><b className="stat-figure text-ink-900">{money(data?.summary?.gross)}</b></div>
    ...
    <AdminRailCard title="Reporting period" subtitle="Change the accounting window without leaving the report.">
    ...
    <AdminRailCard title="Wallet-funded usage" subtitle="Non-additive usage already funded by wallet value." action={<WalletCards size={15} className="text-midnight"/>}>
      <div className="space-y-2">
        ...
        <div className="admin-rail-stat"><span className="text-slate-soft">Wallet revenue*</span><b className="stat-figure text-ink-900">{money(data?.summary?.walletRevenue)}</b></div>
      </div>
    </AdminRailCard>
    <AdminRailCard title="Period context" subtitle="Current reporting window and accounting note.">
      ...
      <p className="mt-3 text-[9px] leading-4 text-slate-soft">Cash received is counted once. Wallet-funded usage remains non-additive.</p>
    </AdminRailCard>
  </>
  ...
  return <AdminPageWorkspace aside={earningsRail}><h1 className="sr-only">Earnings</h1>
    ...
    <div className="admin-metric-grid mb-3">
      <AdminMetricCard label="Gross cash received" value={money(data?.summary?.gross)} hint="Cash ledger for this period" icon={CircleDollarSign} tone="success"/>
      ...
      <AdminMetricCard label="Net income" value={money(data?.summary?.net)} hint="Gross less expenses and provisions" icon={TrendingUp} tone="success"/>
    </div>
    ...
    <section className="earnings-primary-actions mb-3 grid gap-3 xl:grid-cols-2">
      <div className="overview-card flex flex-wrap items-center justify-between gap-3 px-4 py-3">
        <div className="min-w-0"><p className="eyebrow">Reporting tools</p><p className="mt-0.5 text-[11px] text-slate-soft">Generate or refresh the selected reporting period.</p></div>
      ...
      <div className="overview-card flex flex-wrap items-center justify-between gap-3 px-4 py-3">
        <div className="min-w-0"><p className="eyebrow">Expense controls</p><p className="mt-0.5 text-[11px] text-slate-soft">Manage one-time costs and recurring provisions.</p></div>
        <div className="flex flex-wrap items-center gap-2">
    ...
    <section className="overview-card p-4">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div><p className="eyebrow">Revenue sources</p><h2 className="mt-1 text-base font-semibold tracking-[-0.015em] text-ink-900">Revenue breakdown</h2><p className="mt-1 text-[11px] text-slate-soft">Cash received once. Wallet spending is not counted again.</p></div>
    ...
    <section className="earnings-expense-log overview-card mt-3 overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-surface-line px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="eyebrow">Operating costs</p><h2 className="mt-1 text-base font-semibold tracking-[-0.015em] text-ink-900">Expense logs</h2><p className="mt-1 text-[11px] text-slate-soft">Primary accounting log for custom expenses, fixed costs, and operational provisions.</p></div>
