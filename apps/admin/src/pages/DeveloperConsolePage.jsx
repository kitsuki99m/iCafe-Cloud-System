...
export default function DeveloperConsolePage({standalone=false}){
  ...
  async function act(action,payload={}){
    ...
    try{
      ...
      if(result?.activationLink){setActivationLink(result.activationLink);setNotice('Activation link generated. Copy it below if email delivery is unavailable.')}
  ...
  const content=<div className={standalone?'mx-auto w-full max-w-[1440px] p-3 sm:p-6':'space-y-4'}>
    ...
    <div className="grid min-h-[520px] gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
      ...
      <section className="min-w-0 rounded-2xl border border-surface-line bg-surface p-3 sm:p-4">{selected?<>
        ...
        {selected.subscription_status&&<div className="mt-3 rounded-xl border border-surface-line bg-surface-raised/40 p-3 text-xs text-slate-soft"><span className="font-semibold text-ink-900">Subscription:</span> {selected.subscription_plan||'starter'} · {selected.subscription_status}{selected.grace_until?` · grace until ${formatDate(selected.grace_until)}`:''}</div>}
        ...
        {selected.status==='invited'&&<div className="mt-4 space-y-3 rounded-xl border border-gold/25 bg-gold/5 p-3 text-xs leading-5 text-slate-soft"><p>Invitation created {formatDate(selected.invite_sent_at)}. The owner sets their own password after opening the invitation/activation link.</p><div className="grid gap-2 sm:grid-cols-2"><Button variant="ghost" size="sm" icon={Link2} disabled={Boolean(busy)} onClick={copyActivationLink}>{busy==='copy_activation_link'?'Generating…':'Copy activation link'}</Button><Button variant="danger" size="sm" icon={Ban} disabled={Boolean(busy)} onClick={()=>openConfirm('cancel_invite')}>Cancel invite</Button></div></div>}
