...
export default function DeveloperConsolePage({standalone=false}){
  ...
  useEffect(()=>{
    ...
    setUltraStationLimit(String(next==='ultra'?(selected.subscription_max_stations||Math.max(ultraFloor,Number(selected.expected_station_count)||ultraFloor)):(selected.subscription_max_stations||packageDefinition(next,packageCatalog).maxStations||50)))
  ...
  async function savePricingEditor(){
    ...
    try{
      ...
      for(const pkg of pricingDraft){
        ...
        if(!Number.isFinite(monthlyPrice)||monthlyPrice<0)throw new Error(`${pkg.label} monthly price is invalid.`)
        await cloudDeveloperRegistrations('update_package',{packageId:pkg.id,maxStations,monthlyPrice,description:pkg.description||null})
      }
      await cloudDeveloperRegistrations('update_pricing_settings',{deploymentFeeMin:deploymentMin,deploymentFeeMax:deploymentMax,quoteValidDays:quoteDays})
      await load();setPricingOpen(false);setNotice('Pricing catalog updated. New quotations and package suggestions now use these values.')
  ...
  function openQuote(){
    ...
    setQuoteDraft({packageId:pkg.id,stationCount:String(selected.expected_station_count||1),branchCount:'1',monthlyPrice:String(pkg.monthlyPrice??0),deploymentFeePerBranch:String(pricingSettings.deployment_fee_min??2500),validDays:String(pricingSettings.quote_valid_days??14),message:''})
    setQuoteOpen(true);setError('')
  }
  function setQuotePackage(packageId){
    const pkg=packageDefinition(packageId,packageCatalog)
    setQuoteDraft(current=>({...current,packageId:pkg.id,monthlyPrice:String(pkg.monthlyPrice??0)}))
  }
  async function sendQuote(){
    ...
    if(!Number.isInteger(branchCount)||branchCount<1){setError('Quotation branch count must be at least 1.');return}
  ...
  const content=<div className={standalone?'developer-console-page developer-console-standalone':'developer-console-page'}>
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex w-full flex-wrap gap-2 sm:w-auto"><Button className="flex-1 sm:flex-none" variant="ghost" size="sm" icon={Settings2} onClick={openPricingEditor}>Pricing</Button><Button className="flex-1 sm:flex-none" variant="ghost" size="sm" icon={RefreshCw} onClick={load} disabled={loading}>{loading?'Refreshing…':'Refresh'}</Button>{standalone&&<Button className="flex-1 sm:flex-none" variant="ghost" size="sm" icon={LogOut} onClick={logout}>Sign out</Button>}</div>
    </div>
    ...
    <div className="grid min-h-[520px] gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
      ...
      <section className="min-w-0 rounded-2xl border border-surface-line bg-surface p-3 sm:p-4">{selected?<>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gold/20 bg-gold/5 p-3"><div><p className="text-xs font-semibold text-ink-900">Customer quotation</p><p className="mt-1 text-[10px] leading-4 text-slate-soft">Send a branded Aezakmi estimate before the owner decides which package fits.</p></div><Button variant="subtle" size="sm" icon={Send} disabled={Boolean(busy)} onClick={openQuote}>Send quotation</Button></div>
    ...
    <Modal open={pricingOpen} onClose={()=>!busy&&setPricingOpen(false)} busy={busy==='save_pricing'} maxWidth="max-w-3xl" eyebrow="Platform pricing" title="Aezakmi packages & deployment pricing" footer={<><Button variant="ghost" disabled={Boolean(busy)} onClick={()=>setPricingOpen(false)}>Cancel</Button><Button variant="primary" disabled={Boolean(busy)} onClick={savePricingEditor}>{busy==='save_pricing'?'Saving…':'Save pricing'}</Button></>}>
      <div className="space-y-4"><p className="text-xs leading-5 text-slate-soft">These are the defaults used for new quotations and package suggestions. Existing businesses keep their assigned station cap until you explicitly change their package.</p><div className="grid gap-3 sm:grid-cols-2">{pricingDraft.map((pkg,index)=><div key={pkg.id} className="rounded-xl border border-surface-line bg-surface-raised/35 p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-ink-900">{pkg.label}</p><span className="rounded-full bg-gold/10 px-2 py-1 text-[9px] font-semibold uppercase text-gold-dim">{pkg.id}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><label><span className="eyebrow mb-1 block">Monthly ₱</span><input type="number" min="0" value={pkg.monthlyPrice} onChange={e=>setPricingDraft(rows=>rows.map((row,i)=>i===index?{...row,monthlyPrice:e.target.value}:row))} className="min-h-10 w-full rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-ink-900"/></label><label><span className="eyebrow mb-1 block">PC limit</span>{pkg.id==='ultra'?<div className="flex min-h-10 items-center rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-slate-soft">{ultraFloor}+ · custom per business</div>:<input type="number" min="1" max="10000" value={pkg.maxStations} onChange={e=>setPricingDraft(rows=>rows.map((row,i)=>i===index?{...row,maxStations:e.target.value}:row))} className="min-h-10 w-full rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-ink-900"/>}</label></div><label className="mt-2 block"><span className="eyebrow mb-1 block">Description</span><input value={pkg.description||''} onChange={e=>setPricingDraft(rows=>rows.map((row,i)=>i===index?{...row,description:e.target.value}:row))} className="min-h-10 w-full rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-ink-900"/></label></div>)}</div><div className="rounded-xl border border-surface-line bg-surface-raised/35 p-3"><p className="text-xs font-semibold text-ink-900">Initial deployment & quotation defaults</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><label><span className="eyebrow mb-1 block">Minimum / branch ₱</span><input type="number" min="0" value={pricingSettingsDraft.deploymentFeeMin} onChange={e=>setPricingSettingsDraft(v=>({...v,deploymentFeeMin:e.target.value}))} className="min-h-10 w-full rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-ink-900"/></label><label><span className="eyebrow mb-1 block">Maximum / branch ₱</span><input type="number" min="0" value={pricingSettingsDraft.deploymentFeeMax} onChange={e=>setPricingSettingsDraft(v=>({...v,deploymentFeeMax:e.target.value}))} className="min-h-10 w-full rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-ink-900"/></label><label><span className="eyebrow mb-1 block">Quote valid days</span><input type="number" min="1" max="90" value={pricingSettingsDraft.quoteValidDays} onChange={e=>setPricingSettingsDraft(v=>({...v,quoteValidDays:e.target.value}))} className="min-h-10 w-full rounded-lg border border-surface-line bg-soft-white px-3 text-xs text-ink-900"/></label></div></div></div>
    </Modal>
