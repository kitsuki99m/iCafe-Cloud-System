  ...
  const content=<div className={standalone?'developer-console-page developer-console-standalone':'developer-console-page'}>
    ...
    <div className="grid min-h-[520px] items-start gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
        ...
        <div className="mt-3 flex flex-wrap gap-2.5 pt-2.5 pb-1 px-1.5">{FILTERS.map(key=>{
          ...
          return (
            ...
            >
              ...
              {count > 0 && (
                <span
                  className={`absolute -top-1.5 -right-1.5 z-10 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-bold leading-none ring-2 ring-surface shadow-xs ${
                    isActive
      ...
      <section className="min-w-0 rounded-2xl border border-surface-line bg-surface p-3 sm:p-4">{selected?<>
        ...
        {selected.phone&&<p className="mt-3 text-xs text-slate-soft">Phone: <span className="text-ink-900">{selected.phone}</span></p>}{selected.note&&<div className="mt-3 rounded-xl border border-surface-line bg-surface-raised/60 p-3 text-xs leading-5 text-slate-soft">{selected.note}</div>}
        ...
        <label className="mt-4 block"><span className="eyebrow mb-2 block">Developer notes</span><textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3} className="w-full rounded-xl border border-surface-line bg-surface p-3 text-xs text-ink-900 focus:outline-none focus:border-gold/50" placeholder="Internal review note / information requested"/></label>
