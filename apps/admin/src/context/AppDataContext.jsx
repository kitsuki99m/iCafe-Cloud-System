...
import { isCloudAdmin, cloudGetBranchStatus, cloudBranchId } from '../lib/cloudClient.js'
  ...
  useEffect(() => {
    ...
    if (isCloudAdmin()) {
      ...
      const cloudRefresh=async()=>{
        try{const edge=await cloudGetBranchStatus();if(active)setState(current=>({...current,realtimeConnected:Boolean(edge?.last_seen_at)&&Date.now()-new Date(edge.last_seen_at).getTime()<30000}))}catch{if(active)setState(current=>({...current,realtimeConnected:false}))}
        await refresh()
      }
