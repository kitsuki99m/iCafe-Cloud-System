...
export default function BulkActionsDropdown({ items = [], onAction, className = '' }) {
  ...
  return <div className={className}>
    <button
      ...
      className="inline-flex items-center gap-1.5 rounded-xl border border-surface-line bg-surface px-3 py-2 text-xs font-semibold text-ink-900 transition-colors hover:bg-surface-raised"
    >
    ...
    <AnchoredPopover open={open} anchorRef={trigger} onClose={() => setOpen(false)} className="w-64" ariaLabel="Bulk actions">
      ...
      {items.map((item) => {
        ...
        return <button
          ...
          className="flex w-full items-start gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-surface-raised disabled:cursor-not-allowed disabled:opacity-45"
        >
