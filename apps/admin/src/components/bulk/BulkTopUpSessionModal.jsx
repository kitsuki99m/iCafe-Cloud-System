...
export default function BulkTopUpSessionModal({ open, targets = [], ratePlans = [], onClose, onConfirm }) {
  ...
  return <Modal open={open} onClose={onClose} busy={saving} onSubmit={submit} eyebrow="Bulk action" title="Top Up Session Time" footer={<><Button variant="ghost" disabled={saving} onClick={onClose}>Cancel</Button><Button variant="primary" disabled={!valid || saving} onClick={submit}>{saving ? 'Processing…' : 'Add Time'}</Button></>}><div className="space-y-4"><BulkTargetPicker items={targets} selectedIds={selected} onChange={setSelected} searchPlaceholder="Search active sessions"/><div><label className="eyebrow mb-1.5 block">Rate plan</label><select value={planId} onChange={(event) => setPlanId(event.target.value)} className="w-full rounded-lg border border-surface-line bg-ink px-3 py-2 text-sm text-ink-900">{eligibleRatePlans.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>{!isPackage && <div><label className="eyebrow mb-1.5 block">Amount per session</label><div className="mb-2 grid grid-cols-4 gap-1.5">{[5, 10, 15, 20].map((p) => (<button type="button" key={p} onClick={() => setAmount(String(p))} className={`rounded-lg border px-2 py-1.5 text-xs font-medium transition-colors ${Number(amount) === p ? 'border-gold/50 bg-gold/10 text-gold-dim' : 'border-surface-line text-slate-soft hover:text-ink-900'}`}>₱{p}</button>))}</div><NumericInput min={plan?.minAmount || 1} step="0.001" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" className="w-full rounded-lg border border-surface-line bg-ink px-3 py-2 text-sm text-ink-900"/></div>}{error && <p className="rounded-lg border border-ember/30 bg-ember/10 px-3 py-2 text-xs text-ember-dim">{error}</p>}</div></Modal>
}
