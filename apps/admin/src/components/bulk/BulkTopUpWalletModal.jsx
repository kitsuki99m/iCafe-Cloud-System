...
export default function BulkTopUpWalletModal({ open, targets = [], onClose, onConfirm }) {
  ...
  return <Modal open={open} onClose={onClose} busy={saving} onSubmit={submit} eyebrow="Bulk action" title="Top Up Wallets" footer={<><Button variant="ghost" disabled={saving} onClick={onClose}>Cancel</Button><Button variant="primary" disabled={!valid || saving} onClick={submit}>{saving ? 'Processing…' : `Top Up ${selected.size || ''}`}</Button></>}><div className="space-y-4"><BulkTargetPicker items={targets} selectedIds={selected} onChange={setSelected} searchPlaceholder="Search members"/><div><label className="eyebrow mb-1.5 block">Amount per member</label><NumericInput min="1" step="0.001" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" className="w-full rounded-lg border border-surface-line bg-ink px-3 py-2 text-sm text-ink-900 focus:outline-none focus:border-gold/50"/></div>{error && <p className="rounded-lg border border-ember/30 bg-ember/10 px-3 py-2 text-xs text-ember-dim">{error}</p>}</div></Modal>
}
