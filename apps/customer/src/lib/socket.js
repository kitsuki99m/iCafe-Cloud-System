...
export function getSocket() {
  if (!socket) {
    socket = io(socketOrigin(), {
      ...
      auth: { token: getToken() || undefined, clientIp: stationIp(), stationToken:stationToken() },
      autoConnect: false,
      reconnection: true,
...
export function connectSocket() {
  const s = getSocket()
  s.auth = { token: getToken() || undefined, clientIp: stationIp(), stationToken:stationToken() }
