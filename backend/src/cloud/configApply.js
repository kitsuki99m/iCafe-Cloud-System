...
function upsertRuntimeSession(row){
  ...
  db.prepare(`INSERT INTO computer_sessions(id,member_id,pc_id,rate_plan_id,customer_name,billing_type,amount_paid,prepaid_seconds,postpaid_rate_per_minute,settlement_method,started_at,expires_at,last_heartbeat_at,ended_at,end_reason,saved_remaining_seconds,unsettled_amount_due,settlement_pending,status)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET member_id=excluded.member_id,pc_id=excluded.pc_id,rate_plan_id=excluded.rate_plan_id,customer_name=excluded.customer_name,billing_type=excluded.billing_type,amount_paid=excluded.amount_paid,prepaid_seconds=excluded.prepaid_seconds,postpaid_rate_per_minute=excluded.postpaid_rate_per_minute,settlement_method=excluded.settlement_method,started_at=excluded.started_at,expires_at=excluded.expires_at,last_heartbeat_at=excluded.last_heartbeat_at,ended_at=excluded.ended_at,end_reason=excluded.end_reason,saved_remaining_seconds=excluded.saved_remaining_seconds,unsettled_amount_due=excluded.unsettled_amount_due,settlement_pending=excluded.settlement_pending,status=excluded.status`)
    .run(id,row.member_id||null,row.pc_id||null,row.rate_plan_id||null,String(row.customer_name||'Guest'),String(row.billing_type||'prepaid'),row.amount_paid==null?null:Number(row.amount_paid),row.prepaid_seconds==null?null:Number(row.prepaid_seconds),row.postpaid_rate_per_minute==null?null:Number(row.postpaid_rate_per_minute),row.settlement_method||null,row.started_at||nowIso(),row.expires_at||null,row.last_heartbeat_at||null,row.ended_at||null,data.endReason||data.end_reason||row.end_reason||null,data.savedRemainingSeconds??data.saved_remaining_seconds??row.saved_remaining_seconds??null,data.unsettledAmountDue??data.unsettled_amount_due??row.unsettled_amount_due??null,(data.settlementPending??data.settlement_pending??row.settlement_pending)?1:0,String(row.status||'active'))
}
