...
const recent=(at,ms=90000)=>!!at&&Date.now()-new Date(at).getTime()<ms
...
export async function syncCloudNow({reason='manual'}={}){
  ...
  running=(async()=>{
    ...
    try{
      const r=await syncEdge({softwareVersion:env.edgeVersion,snapshot:{...buildEdgeSnapshot(),syncReason:reason,outboxPending:pending.length},configVersion:cfg.version,runtimeCursor:runtimeCursor||null,events,commandAcks:acks.map(commandAck)}),at=r.receivedAt||nowIso()
