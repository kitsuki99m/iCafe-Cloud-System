...
// Remote station controls are process-scoped. Cancel any inherited command so
// a backend restart can never replay stale lock/power/session-control work.
...
function updateStationSoftwareTelemetry(pcId, payload = {}) {
  ...
  if (!version) return
  db.prepare('UPDATE pcs SET customer_version=?,updated_at=updated_at WHERE id=?').run(version,pcId)
}
