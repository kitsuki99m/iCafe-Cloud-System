...
export function migrate() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS cloud_member_credentials (
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_cloud_member_credentials_username ON cloud_member_credentials(username_ci);

    CREATE TABLE IF NOT EXISTS cloud_customer_auth_sessions (
      token_hash TEXT PRIMARY KEY,
      member_id TEXT NOT NULL,
      station_device_id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_cloud_customer_auth_member ON cloud_customer_auth_sessions(member_id,expires_at);
    CREATE INDEX IF NOT EXISTS idx_cloud_customer_auth_station ON cloud_customer_auth_sessions(local_station_id,expires_at);
    ...
    CREATE TABLE IF NOT EXISTS computer_sessions (
      ...
      ended_at TEXT,
      end_reason TEXT,
      saved_remaining_seconds INTEGER,
      unsettled_amount_due REAL,
  ...
  if (!pcCols.includes('customer_version')) db.exec('ALTER TABLE pcs ADD COLUMN customer_version TEXT')
  ...
  if (!sessionCols.includes('end_reason')) db.exec('ALTER TABLE computer_sessions ADD COLUMN end_reason TEXT')
  if (!sessionCols.includes('saved_remaining_seconds')) db.exec('ALTER TABLE computer_sessions ADD COLUMN saved_remaining_seconds INTEGER')
  if (!sessionCols.includes('unsettled_amount_due')) db.exec('ALTER TABLE computer_sessions ADD COLUMN unsettled_amount_due REAL')
  if (!sessionCols.includes('settlement_pending')) db.exec('ALTER TABLE computer_sessions ADD COLUMN settlement_pending INTEGER NOT NULL DEFAULT 0')
  ...
  db.exec(`
    ...
    CREATE TRIGGER cloud_members_insert AFTER INSERT ON members WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN
    ...
    CREATE TRIGGER cloud_members_update AFTER UPDATE ON members WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN
      INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'member.upsert','member',NEW.id,json_object('id',NEW.id,'member_code',NEW.member_code,'name',NEW.name,'username',NEW.username,'birthdate',NEW.birthdate,'phone',NEW.phone,'email',NEW.email,'tier',NEW.tier,'wallet_balance',NEW.wallet_balance,'session_seconds_remaining',NEW.session_seconds_remaining,'status',NEW.status,'pc_id',NEW.pc_id,'pc_ip',NEW.pc_ip,'created_at',NEW.created_at,'updated_at',NEW.updated_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
    CREATE TRIGGER cloud_members_delete AFTER DELETE ON members WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN
    ...
    CREATE TRIGGER cloud_pcs_insert AFTER INSERT ON pcs WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN
    ...
    CREATE TRIGGER cloud_pcs_update AFTER UPDATE ON pcs WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN
      INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'station.upsert','station',NEW.id,json_object('id',NEW.id,'pc_number',NEW.pc_number,'label',NEW.label,'ip_address',NEW.ip_address,'mac_address',NEW.mac_address,'spec',NEW.spec,'status',NEW.status,'created_at',NEW.created_at,'updated_at',NEW.updated_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
    CREATE TRIGGER cloud_pcs_delete AFTER DELETE ON pcs WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN
    ...
    CREATE TRIGGER cloud_sessions_insert AFTER INSERT ON computer_sessions WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'session.upsert','session',NEW.id,json_object('id',NEW.id,'member_id',NEW.member_id,'pc_id',NEW.pc_id,'rate_plan_id',NEW.rate_plan_id,'customer_name',NEW.customer_name,'billing_type',NEW.billing_type,'amount_paid',NEW.amount_paid,'prepaid_seconds',NEW.prepaid_seconds,'postpaid_rate_per_minute',NEW.postpaid_rate_per_minute,'settlement_method',NEW.settlement_method,'started_at',NEW.started_at,'expires_at',NEW.expires_at,'last_heartbeat_at',NEW.last_heartbeat_at,'ended_at',NEW.ended_at,'end_reason',NEW.end_reason,'saved_remaining_seconds',NEW.saved_remaining_seconds,'unsettled_amount_due',NEW.unsettled_amount_due,'settlement_pending',NEW.settlement_pending,'status',NEW.status),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
    CREATE TRIGGER cloud_sessions_update AFTER UPDATE ON computer_sessions WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'session.upsert','session',NEW.id,json_object('id',NEW.id,'member_id',NEW.member_id,'pc_id',NEW.pc_id,'rate_plan_id',NEW.rate_plan_id,'customer_name',NEW.customer_name,'billing_type',NEW.billing_type,'amount_paid',NEW.amount_paid,'prepaid_seconds',NEW.prepaid_seconds,'postpaid_rate_per_minute',NEW.postpaid_rate_per_minute,'settlement_method',NEW.settlement_method,'started_at',NEW.started_at,'expires_at',NEW.expires_at,'last_heartbeat_at',NEW.last_heartbeat_at,'ended_at',NEW.ended_at,'end_reason',NEW.end_reason,'saved_remaining_seconds',NEW.saved_remaining_seconds,'unsettled_amount_due',NEW.unsettled_amount_due,'settlement_pending',NEW.settlement_pending,'status',NEW.status),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;

    DROP TRIGGER IF EXISTS cloud_wallet_insert; CREATE TRIGGER cloud_wallet_insert AFTER INSERT ON wallet_transactions WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'wallet_ledger.insert','wallet_transaction',NEW.id,json_object('id',NEW.id,'member_id',NEW.member_id,'type',NEW.type,'amount',NEW.amount,'balance_before',NEW.balance_before,'balance_after',NEW.balance_after,'reference_type',NEW.reference_type,'reference_id',NEW.reference_id,'created_at',NEW.created_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
    DROP TRIGGER IF EXISTS cloud_session_time_insert; CREATE TRIGGER cloud_session_time_insert AFTER INSERT ON session_time_transactions WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'session_time_ledger.insert','session_time_transaction',NEW.id,json_object('id',NEW.id,'type',NEW.type,'member_id',NEW.member_id,'counterparty_member_id',NEW.counterparty_member_id,'computer_session_id',NEW.computer_session_id,'seconds',NEW.seconds,'reference_id',NEW.reference_id,'created_at',NEW.created_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;

    DROP TRIGGER IF EXISTS cloud_topups_insert; DROP TRIGGER IF EXISTS cloud_topups_update;
    CREATE TRIGGER cloud_topups_insert AFTER INSERT ON top_up_requests WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'top_up.upsert','top_up',NEW.id,json_object('id',NEW.id,'member_id',NEW.member_id,'pc_id',NEW.pc_id,'amount',NEW.amount,'payment_method',NEW.payment_method,'ref_no',NEW.ref_no,'status',NEW.status,'requested_at',NEW.requested_at,'processed_at',NEW.processed_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
    ...
    CREATE TRIGGER cloud_extensions_insert AFTER INSERT ON session_extensions WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'session_extension.upsert','session_extension',NEW.id,json_object('id',NEW.id,'computer_session_id',NEW.computer_session_id,'member_id',NEW.member_id,'rate_plan_id',NEW.rate_plan_id,'amount',NEW.amount,'minutes_added',NEW.minutes_added,'payment_method',NEW.payment_method,'status',NEW.status,'requested_at',NEW.requested_at,'confirmed_at',NEW.confirmed_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
    CREATE TRIGGER cloud_extensions_update AFTER UPDATE ON session_extensions WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'session_extension.upsert','session_extension',NEW.id,json_object('id',NEW.id,'computer_session_id',NEW.computer_session_id,'member_id',NEW.member_id,'rate_plan_id',NEW.rate_plan_id,'amount',NEW.amount,'minutes_added',NEW.minutes_added,'payment_method',NEW.payment_method,'status',NEW.status,'requested_at',NEW.requested_at,'confirmed_at',NEW.confirmed_at),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;

    DROP TRIGGER IF EXISTS cloud_revenue_insert; CREATE TRIGGER cloud_revenue_insert AFTER INSERT ON revenue_events WHEN EXISTS(SELECT 1 FROM cloud_identity WHERE id=1) AND COALESCE((SELECT value FROM sync_state WHERE key='cloud_apply_in_progress'),'0')<>'1' BEGIN INSERT INTO sync_outbox(id,event_type,entity_type,entity_id,payload_json,occurred_at) VALUES(lower(hex(randomblob(16))),'revenue.insert','revenue_event',NEW.id,json_object('id',NEW.id,'event_type',NEW.event_type,'source_type',NEW.source_type,'source_id',NEW.source_id,'amount_centavos',NEW.amount_centavos,'occurred_at',NEW.occurred_at,'category',NEW.category,'payment_method',NEW.payment_method,'member_id',NEW.member_id,'pc_id',NEW.pc_id,'metadata',NEW.metadata,'reversed_event_id',NEW.reversed_event_id),strftime('%Y-%m-%dT%H:%M:%fZ','now')); END;
  ...
  if (appCount === 0) {
    const defaultApps = [
      { id: 'app-steam', name: 'Steam', category_name: 'Online Games', icon: '/assets/launcher/steam.webp', protocol_url: 'steam://', executable_path: 'steam.exe', sort_order: 10 },
      { id: 'app-riot', name: 'Riot / Valorant', category_name: 'Online Games', icon: '/assets/launcher/valorant.webp', protocol_url: 'riotclient://', executable_path: 'RiotClientServices.exe', sort_order: 20 },
      { id: 'app-epic', name: 'Epic Games', category_name: 'Online Games', icon: '/assets/launcher/epicgames.webp', protocol_url: 'com.epicgames.launcher://', executable_path: 'EpicGamesLauncher.exe', sort_order: 30 },
      { id: 'app-roblox', name: 'Roblox', category_name: 'Online Games', icon: '/assets/launcher/roblox.webp', protocol_url: 'roblox://', executable_path: 'RobloxPlayerLauncher.exe', sort_order: 40 },
      { id: 'app-dota2', name: 'Dota 2', category_name: 'Online Games', icon: '/assets/launcher/dota2.webp', protocol_url: 'steam://rungameid/570', executable_path: 'dota2.exe', sort_order: 50 },
      { id: 'app-lol', name: 'League of Legends', category_name: 'Online Games', icon: '/assets/launcher/lol.webp', protocol_url: 'riotclient://launch/league_of_legends', executable_path: 'LeagueClient.exe', sort_order: 60 },
      { id: 'app-cs2', name: 'Counter-Strike 2', category_name: 'Online Games', icon: '/assets/launcher/cs2.webp', protocol_url: 'steam://rungameid/730', executable_path: 'cs2.exe', sort_order: 70 },
      { id: 'app-genshin', name: 'Genshin Impact', category_name: 'Online Games', icon: '/assets/launcher/genshin.webp', executable_path: 'GenshinImpact.exe', sort_order: 80 },
      { id: 'app-minecraft', name: 'Minecraft', category_name: 'Offline Games', icon: '/assets/launcher/minecraft.webp', executable_path: 'Minecraft.exe', sort_order: 90 },
      { id: 'app-chrome', name: 'Google Chrome', category_name: 'Surfing & Browsers', icon: '/assets/launcher/chrome.webp', executable_path: 'chrome.exe', sort_order: 100 },
      { id: 'app-edge', name: 'Microsoft Edge', category_name: 'Surfing & Browsers', icon: '/assets/launcher/edge.webp', executable_path: 'msedge.exe', sort_order: 110 },
      { id: 'app-brave', name: 'Brave Browser', category_name: 'Surfing & Browsers', icon: '/assets/launcher/brave.webp', executable_path: 'brave.exe', sort_order: 120 },
      { id: 'app-discord', name: 'Discord', category_name: 'Utilities & Chat', icon: '/assets/launcher/discord.webp', protocol_url: 'discord://', executable_path: 'Discord.exe', sort_order: 130 },
      { id: 'app-spotify', name: 'Spotify', category_name: 'Utilities & Chat', icon: '/assets/launcher/spotify.webp', protocol_url: 'spotify://', executable_path: 'Spotify.exe', sort_order: 140 },
      { id: 'app-obs', name: 'OBS Studio', category_name: 'Utilities & Chat', icon: '/assets/launcher/obs.webp', executable_path: 'obs64.exe', sort_order: 150 },
      { id: 'app-word', name: 'Word', category_name: 'Office & Productivity', icon: '/assets/launcher/word.webp', executable_path: 'WINWORD.EXE', sort_order: 160 },
      { id: 'app-excel', name: 'Excel', category_name: 'Office & Productivity', icon: '/assets/launcher/excel.webp', executable_path: 'EXCEL.EXE', sort_order: 170 },
      { id: 'app-powerpoint', name: 'PowerPoint', category_name: 'Office & Productivity', icon: '/assets/launcher/powerpoint.webp', executable_path: 'POWERPNT.EXE', sort_order: 180 },
      { id: 'app-calc', name: 'Calculator', category_name: 'Utilities & Chat', icon: '/assets/launcher/calculator.webp', executable_path: 'calc.exe', sort_order: 190 },
      { id: 'app-notepad', name: 'Notepad', category_name: 'Utilities & Chat', icon: '/assets/launcher/notepad.webp', executable_path: 'notepad.exe', sort_order: 200 },
    ];
