...
export function migrate() {
  ...
  if (!pcCols.includes('customer_update_state')) db.exec('ALTER TABLE pcs ADD COLUMN customer_update_state TEXT')
  if (!pcCols.includes('customer_update_version')) db.exec('ALTER TABLE pcs ADD COLUMN customer_update_version TEXT')
  if (!pcCols.includes('customer_update_progress')) db.exec('ALTER TABLE pcs ADD COLUMN customer_update_progress REAL')
  if (!pcCols.includes('customer_update_install_when_idle')) db.exec('ALTER TABLE pcs ADD COLUMN customer_update_install_when_idle INTEGER NOT NULL DEFAULT 0')
  if (!pcCols.includes('customer_update_checked_at')) db.exec('ALTER TABLE pcs ADD COLUMN customer_update_checked_at TEXT')
