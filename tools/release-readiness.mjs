...
check(/const ACTIVE_WIDTH = 640/.test(read('apps/customer/electron/main.cjs'))&&/const ACTIVE_HEIGHT = 480/.test(read('apps/customer/electron/main.cjs'))&&/function applyIdleDashboardMode\(\)[\s\S]*setFullScreen\(true\)[\s\S]*setAlwaysOnTop\(true, 'screen-saver'\)/.test(read('apps/customer/electron/main.cjs'))&&/function enterActiveState\(\)[\s\S]*applyActiveWindowMode\(\{ show:true \}\)/.test(read('apps/customer/electron/main.cjs'))&&/function completeSessionStartTransition\(\)[\s\S]*applyActiveWindowMode\(\{ show:true \}\)/.test(read('apps/customer/electron/main.cjs'))&&/function applyActiveWindowMode[\s\S]*unmaximize\(\)[\s\S]*setAlwaysOnTop\(false\)[\s\S]*setSize\(ACTIVE_WIDTH, ACTIVE_HEIGHT, false\)/.test(read('apps/customer/electron/main.cjs')),'Customer no-session mode must stay fullscreen/topmost while member and Guest active sessions open as a non-topmost 640x480 dashboard.')
