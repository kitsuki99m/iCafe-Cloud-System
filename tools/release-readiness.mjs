...
check(/const ACTIVE_WIDTH = 960/.test(read('apps/customer/electron/main.cjs'))&&/const ACTIVE_HEIGHT = 680/.test(read('apps/customer/electron/main.cjs'))&&/function applyIdleDashboardMode\(\)[\s\S]*setFullScreen\(true\)[\s\S]*setAlwaysOnTop\(true, 'screen-saver'\)/.test(read('apps/customer/electron/main.cjs'))&&/function enterActiveState\(\)[\s\S]*applyCompactSessionMode\(\)/.test(read('apps/customer/electron/main.cjs'))&&/function completeSessionStartTransition\(\)[\s\S]*applyCompactSessionMode\(\)/.test(read('apps/customer/electron/main.cjs'))&&/function applyActiveWindowMode[\s\S]*setKiosk\(true\)[\s\S]*setFullScreen\(true\)/.test(read('apps/customer/electron/main.cjs')),'Customer no-session mode must stay fullscreen/topmost while active sessions support compact and expanded dashboard modes.')
