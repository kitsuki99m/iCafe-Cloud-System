...
test('active Customer Electron uses an extra-small upper-right background timer with customer preferences',()=>{
  ...
  assert.match(main,/function applyCompactSessionMode\(\)[\s\S]*keepWindowContentOpaque\(\)/)
  assert.doesNotMatch(main,/applyWindowOpacity\(timerPrefs\.opacity\)/)
  ...
  assert.match(main,/function setTimerPreferences\(patch = \{\}\)[\s\S]*keepWindowContentOpaque\(\)[\s\S]*mainWindow\.hide\(\)/)
  ...
  assert.match(css,/@media \(max-width: 420px\) and \(max-height: 180px\)[\s\S]*#root[\s\S]*background: transparent !important/)
