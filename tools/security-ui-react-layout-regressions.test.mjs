...
test('logout side effects require the exact active auth session and never revoke every session from a stale token',()=>{
  ...
  const section=src.slice(src.indexOf("router.post('/logout'"),src.indexOf("router.get('/me'"))
  assert.match(section,/auth_sessions[\s\S]*jwt_id=\?[\s\S]*user_id=\?[\s\S]*revoked_at IS NULL[\s\S]*expires_at>\?/)
...
test('renderer bearer tokens are session-scoped and legacy persistent tokens are removed',()=>{
  for(const app of ['admin','customer']){
    ...
    assert.match(src,/sessionStorage\.getItem\(['"]aezakmi\.auth\.token['"]\)/)
    assert.match(src,/sessionStorage\.setItem\(['"]aezakmi\.auth\.token['"]/)
    assert.match(src,/localStorage\.removeItem\(['"]aezakmi\.auth\.token['"]\)/)
    assert.doesNotMatch(src,/return localStorage\.getItem\(['"]aezakmi\.auth\.token['"]\)/)
  }
})
...
test('station-bound public customer actions require the station pairing boundary',()=>{
  ...
  for(const route of ['/public/feedback/me','/public/feedback','/public/support','/public/top-ups','/guest/session']){
    const escaped=route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')
    assert.match(api,new RegExp(`router\\.(?:get|post)\\(\\s*["']${escaped}["']\\s*,\\s*requirePairedStation`))
  }
})
...
test('public remote-command and emergency-control acknowledgements require paired station middleware',()=>{
  ...
  assert.match(api,/['"]\/public\/station-control['"][\s\S]{0,100}requirePairedStation/)
  assert.match(api,/router\.patch\(['"]\/public\/station-control\/:id\/ack['"],\s*requirePairedStation/)
  assert.match(operations,/router\.patch\(['"]\/public\/remote-commands\/:id['"],\s*requirePairedStation/)
})

test('shared dialogs trap focus, restore prior focus, and use unique accessible title ids',()=>{
  for(const file of ['apps/admin/src/components/common/Modal.jsx','apps/customer/src/components/common/Modal.jsx']){
    const src=read(file)
...
test('admin request-center mutations are guarded per request and toast timers are cleaned on unmount',()=>{
  const src=read('apps/admin/src/components/admin/AdminNotificationCenter.jsx')
...
test('admin and customer data refreshes cannot commit after identity generation changes',()=>{
  for(const file of ['apps/admin/src/context/AppDataContext.jsx','apps/customer/src/context/AppDataContext.jsx']){
    const src=read(file)
...
test('customer login collapses to one column in the final <=820px cascade and can scroll',()=>{
  ...
  const media=[...css.matchAll(/@media\s*\(max-width:\s*820px\)\s*\{([\s\S]*?)\n\}/g)]
  assert.ok(media.length>=1)
  const final=media.at(-1)[1]
  assert.match(final,/\.customer-login-grid[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)/)
...
test('authenticated remote-command status mutation is staff-only because stations use the paired public acknowledgement route',()=>{
  ...
  assert.match(src,/router\.patch\(['"]\/remote-commands\/:id['"],\s*auth,\s*requireRole\(['"]admin['"]\)/)
})
...
test('setup-pending admin sessions cannot join the private admin Socket.IO room',()=>{
  ...
  const handshake=src.slice(src.indexOf("io.on('connection'"),src.indexOf("// Guest stations"))
  assert.match(handshake,/must_change_credentials/)
  assert.match(handshake,/session\.role === ['"]admin['"][\s\S]{0,160}must_change_credentials[\s\S]{0,160}disconnect\(true\)/)
...
test('admin PIN + Password mode requires both factors and PIN-only setup does not retain the bootstrap password hash',()=>{
  ...
  assert.match(auth,/method === ['"]pin_password['"][\s\S]{0,1000}pin[\s\S]{0,1000}password/)
  assert.match(auth,/auth_method[\s\S]{0,800}verify-admin-credentials|verify-admin-credentials[\s\S]{0,1200}auth_method/)
  const setup=auth.slice(auth.indexOf("router.post('/setup-credentials'"),auth.indexOf("router.post('/logout'"))
  ...
  assert.match(login,/loginAdminPassword\(username\.trim\(\), password, mode === ["']pin_password["'] \? pin : null\)/)
})
...
test('customer lock and power-warning overlays move focus off controls behind the blocking layer',()=>{
  for(const file of ['apps/customer/src/components/common/StationLockedOverlay.jsx','apps/customer/src/components/common/PowerCommandWarning.jsx']){
    const src=read(file)
...
test('stale branding responses cannot mutate module cache or localStorage',()=>{
  for(const app of ['admin','customer']){
    ...
    const guardIndex=src.indexOf('requestId !== requestSequenceRef.current',hookStart)
    ...
    assert.ok(storageIndex>guardIndex,'localStorage persistence must happen after the latest-request guard')
    assert.ok(cacheIndex>guardIndex,'module cache persistence must happen after the latest-request guard')
  }
})
