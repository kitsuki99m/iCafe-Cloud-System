...
async function wakeBranchEdge(admin:SupabaseClient,branchId:string,payload:any={}){try{const{data:edge}=await admin.from('edge_servers').select('realtime_topic_key').eq('branch_id',branchId).is('revoked_at',null).order('last_seen_at',{ascending:false}).limit(1).maybeSingle();if(edge?.realtime_topic_key)await broadcastEdge(edge.realtime_topic_key,payload)}catch{}}
...
const STATION_OFFLINE_AFTER_MS=180_000
...
Deno.serve(async req=>{const pre=preflight(req);if(pre)return pre;let admin:SupabaseClient|null=null;try{const user=await authUser(req);admin=adminClient();const body=await req.json().catch(()=>({})),action=String(body.action||'');
  if(action==='create'){
    ...
    // A deterministic station id makes create retry-safe even when the first
    // response was lost and the UI had to issue a fresh operation key.
    const{data:existingId,error:existingIdError}=await admin.from('branch_stations').select('*').eq('branch_id',branchId).eq('local_id',localId).maybeSingle();if(existingIdError)throw existingIdError;
    if(existingId){const existingNumber=String(existingId.pc_number||'').trim();if(!pcNumber||existingNumber===pcNumber){const replay={success:true,pc:existingId,duplicate:true,recovered:true};if(operationKey){const{error:receiptError}=await admin.from('cloud_operation_receipts').upsert({branch_id:branchId,actor_key:actorKey,operation_key:operationKey,action:'station.create',response:replay},{onConflict:'branch_id,actor_key,operation_key'});if(receiptError)throw receiptError}return json(replay,200)}throw Object.assign(new Error(`A PC with ID ${localId} already exists.`),{status:409,code:'PC_ID_EXISTS'})}
    if(pcNumber){const{data:duplicateNumber,error:duplicateNumberError}=await admin.from('branch_stations').select('*').eq('branch_id',branchId).eq('pc_number',pcNumber).limit(1).maybeSingle();if(duplicateNumberError)throw duplicateNumberError;if(duplicateNumber)throw Object.assign(new Error(`PC - ${pcNumber} already exists.`),{status:409,code:'PC_NUMBER_EXISTS'})}
    // Capacity is checked only after retry recovery; otherwise a successful
    // create at the package limit can be misreported as STATION_LIMIT_REACHED.
    await requireStationCapacity(admin,branch.organization_id);
  ...
  if(action==='session_close'){
    ...
    await wakeBranchEdge(admin,branchId,{kind:'cloud_mutation',entity:'session',action:`session_${disposition}`,sessionId,stationId});
    return json(out)
  }
