-- Aezakmi Cloud — deployment guard for Admin atomic session close.
-- Reinstall the RPC in a fresh migration so a project whose migration history
-- was repaired/manually advanced cannot run a newer station-admin function
-- without the database object required by Pause & Save / Forfeit / Refund.
--
-- This intentionally repeats the CREATE OR REPLACE definition from 00020.
...
comment on function public.aezakmi_admin_close_session(uuid,text,text,text,text) is
  'Atomic Cloud Admin prepaid session close used by station-admin for Pause & Save, Forfeit, and Refund.';
