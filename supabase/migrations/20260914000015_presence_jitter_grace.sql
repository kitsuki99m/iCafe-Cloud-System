-- Production station presence should not flap Offline because one or two
-- one-second Cloud heartbeats arrive late. Cloud/Admin code uses the same
-- ten-second confirmed-offline window. Keep guest restore parity at the SQL
-- authority boundary so a stale client cannot bypass the presence rule.
...
begin
  ...
  if station.station_device_id is not null and (station.cloud_last_seen_at is null or station.cloud_last_seen_at < ts-interval '10 seconds') then
