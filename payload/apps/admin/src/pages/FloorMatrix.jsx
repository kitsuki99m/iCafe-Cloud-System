    ...
    <AdminPageWorkspace>
      ...
      >
        <div className="space-y-4">
          <p className="text-xs leading-5 text-slate-soft">The business owner email is used only to verify enrollment. After pairing, this PC is permanently scoped by organization, branch, and station ID.</p>
          ...
          {stationPairingResult&&<div className="rounded-2xl border border-teal/25 bg-teal/5 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">Station</p><p className="mt-1 text-sm font-semibold text-ink-900">{stationPairingResult.station?.label||stationPairingPcId}</p></div><span className="rounded-full bg-teal/10 px-2.5 py-1 text-[10px] font-semibold text-teal-dim">15 minute code</span></div><div className="mt-4 rounded-xl border border-surface-line bg-surface px-4 py-4 text-center"><p className="eyebrow">Pairing code</p><p className="stat-figure mt-2 select-all font-mono text-2xl font-bold tracking-[0.18em] text-ink-900">{stationPairingResult.pairingCode}</p></div><div className="mt-3 grid gap-2 text-[11px] text-slate-soft sm:grid-cols-2"><div><b className="text-ink-900">Owner email</b><br/><span className="break-all">{stationPairingResult.ownerEmail}</span></div><div><b className="text-ink-900">Expires</b><br/>{stationPairingResult.expiresAt?new Date(stationPairingResult.expiresAt).toLocaleString():'15 minutes'}</div></div></div>}
          ...
          <div className="rounded-xl border border-surface-line bg-surface-raised/50 p-3 text-[11px] leading-5 text-slate-soft"><b className="text-ink-900">On the client PC:</b> open Aezakmi Customer Station → enter the owner email and this one-time code → Connect this PC. Cloud becomes primary immediately; Café Edge remains the LAN fallback.</div>
        </div>
      </Modal>
      ...
      <BulkAddPcModal open={bulkAddOpen} onClose={() => setBulkAddOpen(false)} onCreate={addPc} existingPcs={pcs} />
