...
test('cloud station API relay reuses local Edge business rules and supports auth restore',()=>{
  const cloud=read('supabase/functions/station-api/index.ts')
  const edge=read('backend/src/cloud/stationApi.js')
  const commands=read('backend/src/cloud/commands.js')
  assert.match(cloud,/station_api/)
  assert.match(cloud,/\/auth\\\/\(login\|heartbeat\|logout\|me\|complete-customer-password-setup\)/)
  assert.match(edge,/internalCloudProxySecret/)
